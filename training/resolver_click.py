#!/usr/bin/env python3
"""
Resolver V2: 用 click 交互训练（匹配竞赛评估分布）。

与 resolver_diagnostic.py (35ch bbox) 的核心区别：
1. 交互类型：bbox → click（set_initial_point），匹配 no-bbox 评估场景
2. 支持多轮交互：Round 0 click → forward → error → Round 1 follow-up click → forward
   每轮都产生训练样本，增加数据多样性
3. 输入扩展：35ch → 40ch
   - 32ch decoder features
   - 3ch competition (own_logit, max_competitor, sum_pressure)
   - 2ch click distance (dist_to_nearest_fg_click, dist_to_nearest_bg_click)
   - 1ch image intensity (z-scored)
   - 1ch prev_pred (from previous round, 0 for first round)
   - 1ch round_index (normalized: round/5)
4. 可选 cross-object attention

用法:
  conda activate nnInteractive  # 需要 nnInteractive 环境的 backbone
  python -u training/resolver_click.py --max_files 200 --epochs 30 --device cuda:0
  python -u training/resolver_click.py --max_files 200 --epochs 30 --device cuda:0 --use_attention
"""
import argparse
import random
import sys
...
# scipy must be imported BEFORE numpy._core hack
...
from training.interaction_sim import InteractionManager, generate_point_blob, sample_point_from_error_region, POINT_RADIUS
...
DEFAULT_OUTPUT = PROJECT_ROOT / "experiments" / "resolver_click"

MAX_OBJECTS = 4
CONTESTED_WEIGHT = 10.0
NUM_INTERACTION_ROUNDS = 2  # Round 0 (initial click) + 1 follow-up
IN_CHANNELS = 40  # 32 feat + 3 competition + 2 click_dist + 1 image + 1 prev_pred + 1 round_idx


# ─── Resolver V2 ────────────────────────────────────────────────────────────

class ResolverV2(nn.Module):
    """40ch resolver with optional cross-object attention."""

    def __init__(self, in_ch=IN_CHANNELS, hidden=32, use_attention=False):
        super().__init__()
        self.use_attention = use_attention

        # Per-object spatial feature extractor
        ...
        # Optional cross-object attention
        if use_attention:
        ...
        # Output: refined logit (residual on original)
        self.head = nn.Conv3d(hidden, 1, 1)
        # 初始化 head 接近零 → 初始 refined_logit ≈ 0，退化到 max_prob_sigmoid
        nn.init.zeros_(self.head.weight)
        nn.init.zeros_(self.head.bias)

    def forward_single(self, x):
        """Process single object: (1, in_ch, D, H, W) → (1, hidden, D, H, W)."""
        return self.spatial(x)

    def forward(self, x_list, own_logits=None):
        """
        x_list: list of K tensors, each (1, in_ch, D, H, W)
        own_logits: list of K tensors, each (D, H, W) — original margin logits for residual
        Returns: list of K tensors, each (D, H, W) — refined logits
        """
        K = len(x_list)
        feat_list = [self.forward_single(x) for x in x_list]  # K × (1, hidden, D, H, W)

        if self.use_attention and K > 1:
            hidden = feat_list[0].shape[1]
            D, H, W = feat_list[0].shape[2:]
            N = D * H * W

            # Reshape to (N, K, hidden) for cross-object attention
            stacked = torch.stack(
                [f[0].reshape(hidden, N).T for f in feat_list], dim=1
            )  # (N, K, hidden)
            ...
            for start in range(0, N, chunk_size):
                ...
                chunk = stacked[start:end]  # (chunk, K, hidden)
                ...
                attn_out_chunks.append(chunk)

            stacked = torch.cat(attn_out_chunks, dim=0)  # (N, K, hidden)

            for k in range(K):
                feat_list[k] = stacked[:, k, :].T.reshape(1, hidden, D, H, W)

        # Output head → refined logit
        refined = []
        for k in range(K):
            delta = self.head(feat_list[k])[0, 0]  # (D, H, W)
            if own_logits is not None:
                # Residual: refined = original + learned_delta
                refined.append(own_logits[k] + delta)
            else:
...
def main():
    ...
    parser.add_argument("--rounds", type=int, default=NUM_INTERACTION_ROUNDS,
                        help="Number of interaction rounds per training step")
    parser.add_argument("--use_attention", action="store_true",
                        help="Enable cross-object attention")
    ...
    # Resolver
    resolver = ResolverV2(
        in_ch=IN_CHANNELS, hidden=32, use_attention=args.use_attention
    ).to(device)
    n_params = sum(p.numel() for p in resolver.parameters())
    attn_str = "+attention" if args.use_attention else ""
    print(f"ResolverV2 {IN_CHANNELS}ch{attn_str}: {n_params:,} params")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            ...
            image_np = batch['image'].numpy().squeeze(0)  # (D, H, W) z-scored, remove channel dim
            ...
            # Accumulate clicks and predictions across rounds
            obj_clicks = {oid: {'fg': [], 'bg': []} for oid in oids}
            prev_preds = {oid: np.zeros(spatial, dtype=np.uint8) for oid in oids}

            round_samples = []  # collect (fg_logits, feat_list, click_info, round_idx)

            for round_idx in range(args.rounds):
                fg_logits = []
                feat_list = []
                click_info = []  # per-object click positions for this round

                for oid in oids:
                    gt_binary = (gt_np == oid).astype(np.uint8)
                    mgr = InteractionManager(spatial)

                    # Set prev_pred if not first round
                    if round_idx > 0:
                    ...
                    if round_idx == 0:
                        ...
                        # Record click position (approximate: GT center)
                        coords = np.argwhere(gt_binary > 0)
                        if len(coords) > 0:
                            dt = _edt_fn(gt_binary)
                            center = tuple(np.unravel_index(dt.argmax(), dt.shape))
                            obj_clicks[oid]['fg'].append(center)
                    else:
                        # Follow-up based on prediction error
                        # First, restore previous clicks with decay
                        for c in obj_clicks[oid]['fg']:
                            blob = generate_point_blob(spatial, c, POINT_RADIUS)
                            decay = 0.9 ** round_idx
                            mgr.interactions[3] = np.maximum(mgr.interactions[3], blob * decay)
                        for c in obj_clicks[oid]['bg']:
                            blob = generate_point_blob(spatial, c, POINT_RADIUS)
                            decay = 0.9 ** round_idx
                            mgr.interactions[4] = np.maximum(mgr.interactions[4], blob * decay)

                        # Generate follow-up click
                        fn_region = (gt_binary > 0) & (prev_preds[oid] == 0)
                        fp_region = (gt_binary == 0) & (prev_preds[oid] > 0)

                        if fn_region.sum() > fp_region.sum():
                            center = sample_point_from_error_region(fn_region)
                            if center:
                                obj_clicks[oid]['fg'].append(center)
                                blob = generate_point_blob(spatial, center, POINT_RADIUS)
                                mgr.interactions[3] = np.maximum(mgr.interactions[3], blob)
                        elif fp_region.sum() > 0:
                            center = sample_point_from_error_region(fp_region)
                            if center:
                                obj_clicks[oid]['bg'].append(center)
                                blob = generate_point_blob(spatial, center, POINT_RADIUS)
                                mgr.interactions[4] = np.maximum(mgr.interactions[4], blob)
                    ...
                    fg_logits.append(fg)
                    feat_list.append(captured_features['feat'][0].float().cpu())
                ...
                round_samples.append((fg_logits, feat_list, click_info, round_idx))
            ...
            for fg_logits, feat_list, click_info, round_idx in round_samples:
                # Check if there's any contested region
                fg_masks = [fg > 0 for fg in fg_logits]
                contested_any = False
                for i in range(K):
                    for j in range(i + 1, K):
                        if (fg_masks[i] & fg_masks[j]).any():
                            contested_any = True
                            break
                    if contested_any:
                        break
                ...
                # ── Crop to foreground ROI to save VRAM ──
                ...
                roi_max = np.minimum(fg_coords.max(0) + roi_margin + 1,
                                     [spatial[0], spatial[1], spatial[2]])
                # Cap ROI to max 96³ for VRAM safety
                ...
                roi_sl = tuple(slice(int(mn), int(mx)) for mn, mx in zip(roi_min, roi_max))
                roi_shape = tuple(int(mx - mn) for mn, mx in zip(roi_min, roi_max))

                # Build resolver inputs on ROI only
                resolver_inputs = []
                own_logit_list = []
                ...
                with autocast_ctx():
                    for k in range(K):
                        ...
                        others = [fg_logits[j][roi_sl] for j in range(K) if j != k]
                        ...
                        # Click distance maps (compute on full, crop)
                        fg_dist, bg_dist = compute_click_distance_maps(
                            spatial, click_info[k]['fg'], click_info[k]['bg']
                        )
                        ...
                        img_roi = torch.from_numpy(image_np[roi_sl].astype(np.float32))

                        feat_roi = feat_list[k][:, roi_sl[0], roi_sl[1], roi_sl[2]]
                        ...
                        resolver_in = torch.cat([
                            ...
                            feat_roi,
                            torch.from_numpy(fg_dist_roi).unsqueeze(0),
                            torch.from_numpy(bg_dist_roi).unsqueeze(0),
                            img_roi.unsqueeze(0),
                            pp.unsqueeze(0),
                            round_ch.unsqueeze(0),
                        ], dim=0).unsqueeze(0).to(device)

                        resolver_inputs.append(resolver_in)
                        own_logit_list.append(own.to(device))

                    # Resolver forward (on ROI)
                    refined_list = resolver(resolver_inputs, own_logit_list)

                    # Assembly + loss (on ROI)
                    bg = torch.full(roi_shape, BG_LOGIT, device=device)
                    stacked = torch.stack([bg] + refined_list)

                    gt_roi = gt_np[roi_sl]
                    gt_remapped = torch.zeros(roi_shape, dtype=torch.long, device=device)
                    for new_idx, oid in enumerate(oids):
                        gt_remapped[gt_roi == oid] = new_idx + 1

                    fg_pos = torch.stack([fg_logits[j][roi_sl] > 0 for j in range(K)]).to(device)
                    contested = (fg_pos.sum(0) >= 2)

                    weight_map = torch.ones(roi_shape, dtype=torch.float32, device=device)
            ...
            if n_round_samples > 0:
                ...
                torch.nn.utils.clip_grad_norm_(resolver.parameters(), max_norm=5.0)
                optimizer.step()
    ...
    torch.save({'resolver_state': resolver.state_dict(),
                'config': {'in_ch': IN_CHANNELS, 'hidden': 32,
                           'use_attention': args.use_attention}},
