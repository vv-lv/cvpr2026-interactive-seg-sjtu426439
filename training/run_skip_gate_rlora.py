"""
Skip Gate + Round-Aware LoRA combined training.

- SkipGate: multiplicative gate on high-res skip connections (stages 2,3,4)
- RoundAwareLoRA: round-specific LoRA on decoder stages 2,3
  Group 0: Round 1 (early refinement, aggressive)
  Group 1: Round 2+ (late refinement, conservative)

Usage:
    python -m training.run_skip_gate_rlora --num_files 300 --epochs 3 --gpu 0
"""
import argparse
import os
import random
...
from training.round_aware_lora import (
    apply_round_aware_lora, set_round_group, set_round_lora_bypass,
    get_round_lora_params, RoundAwareLoRAConv3d,
)
...
class SkipGateRLoRATrainer:

    def __init__(self, gpu=0, lr_gate=0.01, lr_lora=3e-4,
                 num_rounds=4, gate_stages=(2, 3, 4),
                 lora_stages='2,3', lora_rank=4, num_round_groups=2):
        ...
        self.gated_decoder = GatedDecoder(
            ...
            gate_input_channels=1).to(self.device)
        n_gate = sum(p.numel() for p in self.gated_decoder.gates.parameters())
        print(f"SkipGate: {n_gate} params on stages {list(gate_stages)}")

        # Round-aware LoRA
        lora_stage_list = [int(s) for s in lora_stages.split(',')]
        n_lora = apply_round_aware_lora(
            self.network.decoder, target_stages=lora_stage_list,
            rank=lora_rank, num_groups=num_round_groups)
        self.network.decoder.to(self.device)
        self.lora_params = get_round_lora_params(self.network)
        for p in self.lora_params:
            p.requires_grad_(True)
        print(f"RoundLoRA: {n_lora} params, {num_round_groups} groups, "
              f"stages {lora_stage_list}")
        print(f"Training rounds: {num_rounds}")

        self.criterion = build_loss(deep_supervision=True).to(self.device)

        self.optimizer = torch.optim.AdamW([
            {'params': list(self.gated_decoder.gates.parameters()),
             'lr': lr_gate, 'weight_decay': 1e-4},
    ...
    def _train_step(self, image_crop, gt_crop, label):
        ...
        for round_idx in range(self.num_rounds):
            ...
            # Set round-aware LoRA group
            if is_active:
                set_round_lora_bypass(self.network, False)
                round_group = 0 if round_idx <= 1 else 1
                set_round_group(self.network, round_group)
            else:
                set_round_lora_bypass(self.network, True)
        ...
        if n_fwd > 0:
            ...
            all_params = list(self.gated_decoder.gates.parameters()) + self.lora_params
    ...
    def save_checkpoint(self, path, epoch, loss):
        ...
        for name, module in self.network.named_modules():
            if isinstance(module, RoundAwareLoRAConv3d):
                for g in range(module.num_groups):
                    lora_state[f'{name}.lora_A.{g}.weight'] = module.lora_A[g].weight.data.cpu()
                    lora_state[f'{name}.lora_B.{g}.weight'] = module.lora_B[g].weight.data.cpu()
        torch.save({
...
def main():
    ...
    parser.add_argument('--num_round_groups', type=int, default=2)
    parser.add_argument('--max_per_dataset', type=int, default=30)
    parser.add_argument('--save_dir', default='experiments/skip_gate_rlora_v1')
    ...
    trainer = SkipGateRLoRATrainer(
        gpu=args.gpu, lr_gate=args.lr_gate, lr_lora=args.lr_lora,
        num_rounds=args.num_rounds,
        gate_stages=[int(s) for s in args.gate_stages.split(',')],
        lora_stages=args.lora_stages, lora_rank=args.lora_rank,
        num_round_groups=args.num_round_groups)
